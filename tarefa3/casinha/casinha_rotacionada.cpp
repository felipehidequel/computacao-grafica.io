...
void rotacionarLinha(int *x0, int *y0, int *x1, int *y1, float anguloGraus,
                     int cx, int cy) {
  float angulo = anguloGraus * M_PI / 180.0;
  ...
  *x0 = (int)(x0_old * cos(angulo) - y0_old * sin(angulo)) + cx;
  *y0 = (int)(x0_old * sin(angulo) + y0_old * cos(angulo)) + cy;
  *x1 = (int)(x1_old * cos(angulo) - y1_old * sin(angulo)) + cx;
  *y1 = (int)(x1_old * sin(angulo) + y1_old * cos(angulo)) + cy;
}
...
int main() {
  ...
  rotacionarLinha(&base_x0, &base_y0, &base_x1, &base_y1, angulo, centro_x,
                  centro_y);
  rotacionarLinha(&teto_x0, &teto_y0, &teto_x1, &teto_y1, angulo, centro_x,
                  centro_y);
  rotacionarLinha(&left_wall_x0, &left_wall_y0, &left_wall_x1, &left_wall_y1,
                  angulo, centro_x, centro_y);
  rotacionarLinha(&rigth_wall_x0, &rigth_wall_y0, &rigth_wall_x1,
                  &rigth_wall_y1, angulo, centro_x, centro_y);
  rotacionarLinha(&telhado_left_x0, &telhado_left_y0, &telhado_left_x1,
                  &telhado_left_y1, angulo, centro_x, centro_y);
  rotacionarLinha(&telhado_rigth_x0, &telhado_rigth_y0, &telhado_rigth_x1,
                  &telhado_rigth_y1, angulo, centro_x, centro_y);
  rotacionarLinha(&base_porta_x0, &base_porta_y0, &base_porta_x1,
                  &base_porta_y1, angulo, centro_x, centro_y);
  rotacionarLinha(&topo_porta_x0, &topo_porta_y0, &topo_porta_x1,
                  &topo_porta_y1, angulo, centro_x, centro_y);
  rotacionarLinha(&porta_left_x0, &porta_left_y0, &porta_left_x1,
                  &porta_left_y1, angulo, centro_x, centro_y);
  rotacionarLinha(&porta_rigth_x0, &porta_rigth_y0, &porta_rigth_x1,
                  &porta_rigth_y1, angulo, centro_x, centro_y);
